...
// Point to your backend (Vite .env: VITE_API_URL=http://localhost:5050)
...
export default function PredictionForTeachers() {
  ...
  const [passMark, setPassMark] = useState(
    () => +(localStorage.getItem("pred.passMark") ?? 40)
  );
  useEffect(
    () => localStorage.setItem("pred.passMark", String(passMark)),
    [passMark]
  );

  // Selected student id ('' = manual)
  const [selectedId, setSelectedId] = useState(
    () => localStorage.getItem("pred.selectedId") ?? ""
  );
  useEffect(
    () => localStorage.setItem("pred.selectedId", selectedId),
    [selectedId]
  );
  ...
  useEffect(
    () => localStorage.setItem("pred.lastForm", JSON.stringify(f)),
    [f]
  );
    ...
    setF((prev) => ({
      ...
      extracurricularCount:
        typeof s.activities === "number"
          ? s.activities
          : prev.extracurricularCount ?? 1,
      parentalSupport: s.parent || "Medium",
      online: !!s.online,
    }));
  }, [selectedId, dataset]);

  // ---------- PREDICTION (via backend) ----------
  const [pred, setPred] = useState({ predicted: 0, passProb: 0, risk: "—" });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const debounceRef = useRef(0);

  // Build backend row from current form
  const currentRow = useMemo(
    () => ({
      id: "manual-1",
      name: f.studentName || "Unnamed Student",
      attendance: Math.max(0, Math.min(1, (f.attendancePct || 0) / 100)),
      hours: +f.studyHours || 0,
      prev: +f.previousGrade || 0,
      final: undefined,
      parent: f.parentalSupport || "Medium",
      online: !!f.online,
      activities: Number.isFinite(+f.extracurricularCount)
        ? +f.extracurricularCount
        : undefined,
    }),
    [f]
  );

  // Debounced auto-predict whenever inputs change
  useEffect(() => {
    window.clearTimeout(debounceRef.current);
    debounceRef.current = window.setTimeout(runPredict, 350);
  ...
  async function runPredict() {
    try {
      setLoading(true);
      setError("");
      const data = await postJSON("/api/predict", { rows: [currentRow] });
      const first = data?.results?.[0];
      const predicted = Math.round((first?.predicted_final ?? 0) * 100) / 100;
      const risk = first?.risk || "—";
      // pass probability: simple scalar if backend doesn't provide it
      const passProb = Math.max(0, Math.min(1, predicted / 100));
      setPred({ predicted, passProb, risk });
    } catch (e) {
  ...
  function addToAtRisk() {
    try {
      ...
      const next = exists
        ? list.map((x) => (x.name === name ? entry : x))
        : [...list, entry];
  ...
  function exportCSV() {
    ...
    a.href = url;
    a.download = `${f.studentName || "student"}_prediction.csv`;
    a.click();
    ...
    <div className="space-y-4 mt-2">
      ...
      <Panel className="no-print">
        ...
        <p className="text-sm opacity-90 mt-2">
          Pick a student from the dataset or choose “Manual entry”. Adjust the
          sliders if needed — the estimate updates instantly.
        </p>
      </Panel>
          ...
          <Field label={`Pass mark (final ≥ ${passMark})`}>
            ...
            >
              {[30, 35, 40, 45, 50].map((p) => (
                <option key={p} className="bg-[#0b1220]" value={p}>
                  {p}
                </option>
              ))}
            </select>
      ...
      <div className="grid xl:grid-cols-3 gap-4">
        ...
        <Panel className="xl:col-span-2 no-print">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <Field label={`Previous Grade (${f.previousGrade})`}>
              <input
                ...
                min="0"
                max="100"
                value={f.previousGrade}
                onChange={set("previousGrade")}
                className="w-full"
              />
            </Field>
            ...
            <Field label={`Extracurricular (count: ${f.extracurricularCount})`}>
              <input
                ...
                min="0"
                max="5"
                value={f.extracurricularCount}
                onChange={set("extracurricularCount")}
                className="w-full"
              />
            </Field>
            ...
            <Field label="Parental Support">
              <select
                value={f.parentalSupport}
                onChange={set("parentalSupport")}
                className="w-full bg-transparent border rounded px-3 py-2"
                style={{ borderColor: panelBorder }}
              >
                {["High", "Medium", "Low"].map((x) => (
                  <option key={x} className="bg-[#0b1220]" value={x}>
                    {x}
                  </option>
                ))}
              </select>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            ...
            {/* Manual refresh (visible when loading or error) */}
            <button
              onClick={runPredict}
              className="px-3 py-2 rounded font-medium"
              style={{ border: `1px solid ${panelBorder}` }}
            >
              {loading ? "Predicting…" : "Re-run Prediction"}
            </button>
          </div>
          ...
          <div className="space-y-3">
              ...
              <div>
                ...
                <div className="mt-2 h-2 w-full rounded bg-[rgba(255,255,255,0.06)] overflow-hidden">
                  <div
                    ...
                    style={{ width: `${passProbPct}%`, background: "#9BE7FF" }}
                  />
                </div>
              </div>
            </KV>
            ...
            <div className="pt-3 border-t" style={{ borderColor: panelBorder }}>
              <div className="text-[11px] opacity-70">
                Estimate blends attendance, study hours, prior grade,
                activities, support, and online recaps. Use alongside teacher
                judgment.
              </div>
            </div>
          </div>
      ...
      <Panel className="no-print">
        <h3 className="font-semibold mb-2 tracking-wide">
          Suggested next steps (2-week plan)
        </h3>
        <ul className="list-disc pl-5 space-y-1 text-sm opacity-90">
          {teacherTips(f).map((t, i) => (
            <li key={i}>{t}</li>
          ))}
        </ul>
        <p className="text-xs opacity-70 mt-2">
          Tip: mark this student as “At-Risk” to include them in your daily
          follow-up list on the Classification tab.
        </p>
      </Panel>
      ...
      <div id="print-sheet" className="print-only">
        ...
        <div className="section">
          ...
          <div className="kv">
            <div>
              <b>Predicted final grade</b>
            </div>
            <div>{pred.predicted}</div>
            <div>
              <b>Chance to pass</b>
            </div>
            <div>{passProbPct}%</div>
        ...
        <div className="section">
          ...
          <div className="kv">
            <div>Attendance</div>
            <div>{f.attendancePct}%</div>
            <div>Study hours / week</div>
            <div>{f.studyHours}</div>
            <div>Previous grade</div>
            <div>{f.previousGrade}</div>
            <div>Extracurricular count</div>
            <div>{f.extracurricularCount}</div>
            <div>Parental support</div>
            <div>{f.parentalSupport}</div>
            <div>Online recap?</div>
            <div>{f.online ? "Yes" : "No"}</div>
          </div>
        </div>
        ...
        <div className="section">
          ...
          <p className="muted" style={{ marginTop: "8pt" }}>
            Note: AI-assisted guidance to support teacher judgment; not a final
            decision.
          </p>
        </div>
...
function teacherTips(f) {
  ...
  if (f.attendancePct < 80)
    out.push("Prioritise attendance this week (target ≥ 85%).");
  if (f.studyHours < 10)
    out.push("Add 2–4 self-study hours (two 1-hour evening blocks).");
  if (f.previousGrade < 60)
    out.push("Schedule 1:1 basics revision; share topic checklist.");
  if (f.extracurricularCount > 2 && f.studyHours < 10)
    out.push("Temporarily limit activities until tests are over.");
  if (f.parentalSupport === "Low")
    out.push("Send weekly plan to parents; ask for quiet, fixed study time.");
  if (!f.online)
    out.push("Assign short recap videos (10–15 mins) after class.");
  if (!out.length)
    out.push("Maintain current habits; set a stretch goal for mock tests.");
  return out;
}
...
// add temporarily anywhere in app
