...
# ---------- Simple demo logic (swap with your model later) ----------
def predict_final(row: StudentIn) -> float:
    # toy regression-ish formula; replace with your trained model
...
def label_student(row: StudentIn) -> (str, list):
    reasons = []
    if row.attendance < 0.6: reasons.append("Low attendance")
    if row.hours < 5: reasons.append("Low study hours")
    if row.prev < 50: reasons.append("Weak prior grade")
...
def cluster(req: PredictRequest):
    # cluster on [attendance%, hours, prev]
